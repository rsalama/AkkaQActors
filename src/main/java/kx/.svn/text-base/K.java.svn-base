...
public class K
{
     ...
     * O(c.k("0N!",c.k("0N!1999.01.01D-1"))); c.close();}catch(Exception
     * e){e.printStackTrace();}}
     */
    public Socket s;
    DataInputStream i;
    ...
    public K(ServerSocket s) throws IOException
    {
        io(s.accept());
        i.read(b = new byte[99]);
        o.write(b, 0, 1);
    } // K k=new c(new ServerSocket(5010));while(true)c.w(2,c.k());

    public K(String h, int p, String u) throws KException, IOException
    {
        B = new byte[2 + ns(u)];
        io(new Socket(h, p));
        J = 0;
    ...
    public K(String h, int p) throws KException, IOException
    {
    ...
    private static class KTime extends Time
    {
        // 19:00:01.000 
        public static DateFormat df = new SimpleDateFormat("HH:mm:ss.SSS");
        public KTime(long time) { super(time); }
        public String toString() { return df.format(this); }
    }
    
    @SuppressWarnings("serial")
    private static class KTimestamp extends Timestamp
    {
        // 2010-04-22 19:00:00.0  // "%1$Y.%1$m.%1$dD%1$H:%1$M:%1$S.%1$N"
        public KTimestamp(long time) { super(time); }
        public String toString() { return String.format("%1$tY.%1$tm.%1$tdD%1$tH:%1$tM:%1$tS.%2$09d", this, this.getNanos()); } // { return df.format(this); }
    }
    
    @SuppressWarnings("serial")
    public static class KException extends Exception
    {
        KException(String s) { super(s); }
    }
    ...
    {
        int i = ri();
        return new KTime(i == ni ? nj : gl(i)); // rs
    }
    ...
    {
        double f = rf();
        return new KTimestamp(Double.isNaN(f) ? nj : gl(k + (long) (8.64e7 * f))); // rs
    }
    ...
    Timestamp rp()
    {
        ...
        Timestamp p = new KTimestamp(j == nj ? j : gl(k + 1000 * d)); // rs
        if (j != nj)
    ...
    String rs() throws UnsupportedEncodingException
    {
        ...
        for (; b[j++] != 0;)
            ;
        return new String(b, i, j - 1 - i, "ISO-8859-1");
    }
    ...
    Object r() throws UnsupportedEncodingException
    {
        ...
        case 11:
            ...
            return S;
        case 12:
            Timestamp[] P = new KTimestamp[n]; // rs
            for (; i < n; i++)
                P[i] = rp();
            return P;
        case 13:
        ...
        case 14:
            ...
            return D;
        case 15:
            Timestamp[] Z = new KTimestamp[n]; // rs
            for (; i < n; i++)
                Z[i] = rz();
            return Z;
        case 16:
    ...
    public static Object[] NULL = { null, new Boolean(false), null, null,
            ...
            new Timestamp(nj), new Timespan(nj), new Minute(ni),
